...
class GoogleMapsScraper:
    ...
    def __init__(self, headless: bool = False, timeout: int = 30000) -> None:
            ...
            try:
                ...
                for _ in range(3):
                    ...
                    try:
                        ...
                        logger.warning("Consent dialog detection failed, trying vision fallback")
                        try:
                            diag_path = self.screenshots_dir / f"consent_{int(time.time())}.png"
                            page.screenshot(path=str(diag_path), full_page=False)
                            logger.info("Saved consent diagnostic screenshot: %s", diag_path)
                            agent = VisionAgent(page, max_steps=2)
                            agent.run_task("Click the cookie consent or privacy dialog dismiss/accept/continue button")
                            consent_handled = True
                        except Exception:
                            pass
                        break
                ...
                # 3) Vision-agent fallback: screenshot → VLM → act
                if not search_done:
                    try:
                        diag_path = self.screenshots_dir / f"search_{int(time.time())}.png"
                        page.screenshot(path=str(diag_path), full_page=False)
                        logger.info("Saved search diagnostic screenshot: %s", diag_path)
                        agent = VisionAgent(page, max_steps=3)
                        agent.run_task(
                            f"Find the Google Maps search input field and type the following query exactly, then press Enter: {query}"
                        )
                        search_done = True
                        logger.info("Search filled via vision agent")
                    except Exception as vision_exc:
                        logger.error("Vision search fallback failed: %s", vision_exc)
                        raise DiscoveryError(
                            f"Could not locate or fill the Google Maps search box after all fallbacks: {vision_exc}"
                        ) from vision_exc
