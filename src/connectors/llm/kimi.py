"""Kimi (Moonshot AI) API connector.

Kimi's API is OpenAI-compatible.  This connector uses the ``openai`` SDK
with a custom ``base_url`` pointing at Kimi's coding endpoint.
"""
...
def _build_messages(context: BusinessContext, preset: dict | None = None) -> list[dict[str, str]]:
    """Build Kimi chat messages from business context."""
    preset = preset or {}
...
class KimiConnector(LLMConnector):
    """Connector for Kimi (Moonshot AI) API."""

    def __init__(self, model: str | None = None) -> None:
        self.settings = get_settings()
        self.model = model or getattr(self.settings, "kimi_model", _DEFAULT_MODEL)
        self._client: Any = None

    def _get_client(self) -> Any:
        """Lazy-load the OpenAI client pointing at Kimi's endpoint."""
        ...
        api_key = getattr(self.settings, "kimi_api_key", "")
        if not api_key:
            raise ConnectorError("KIMI_API_KEY is not configured")
    ...
    def generate_pitch(self, context: BusinessContext, preset: dict | None = None) -> PitchResult:
        """Generate pitch using the Kimi API with JSON mode."""
        ...
        try:
            ...
            msg = f"Kimi API request failed: {exc}"
            if isinstance(exc, _OpenAITimeoutError):
                msg = "Kimi API request timed out"
        ...
        try:
            ...
            raise ConnectorError(f"Kimi response is not valid JSON: {exc}") from exc

        return PitchResult(
            pitch_text=parsed.get("pitch_text", ""),
            membership_idea=parsed.get("membership_idea", ""),
            website_benefits=parsed.get("website_benefits", ""),
            model_used=parsed.get("model_used", "kimi"),
        )

    def health_check(self) -> bool:
        """Check if the Kimi API key is configured and has a valid format."""
        key = getattr(self.settings, "kimi_api_key", "")
        return bool(key) and key.startswith("sk-")

    @classmethod
    def can_use(cls) -> bool:
        """Check if a Kimi API key is present without instantiating a client."""
        return bool(getattr(get_settings(), "kimi_api_key", ""))
