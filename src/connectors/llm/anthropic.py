"""Anthropic API connector."""

from __future__ import annotations

import json
import logging
from typing import Any

from src.config import get_settings
from src.connectors.llm.base import BusinessContext, ConnectorError, LLMConnector, PitchResult

logger = logging.getLogger(__name__)

_DEFAULT_MODEL = "claude-3-5-sonnet-20241022"
_DEFAULT_SYSTEM_PROMPT = (
    "You are a helpful assistant that writes personalized business pitches. "
    "Always respond with valid JSON containing pitch_text, membership_idea, website_benefits, and model_used."
)

try:
    from anthropic import APIError as _AnthropicAPIError
    from anthropic import APITimeoutError as _AnthropicTimeoutError
except ImportError:
    _AnthropicAPIError = type("_AnthropicAPIError", (Exception,), {})
    _AnthropicTimeoutError = type("_AnthropicTimeoutError", (Exception,), {})


def _build_prompt(context: BusinessContext, preset: dict | None = None) -> str:
    """Build Anthropic prompt text from business context."""
    preset = preset or {}
    issues = ", ".join(context.website_issues) if context.website_issues else "none"
    tone = preset.get("tone")
    tone_instruction = f" Use a {tone} tone." if tone else ""

    return (
        f"Business: {context.name} in {context.city}\n"
        f"Type: {context.business_type}\n"
        f"Years in business: {context.years_in_business or 'unknown'}\n"
        f"Has website: {context.has_website}\n"
        f"Website score: {context.website_score or 'none'}\n"
        f"Website issues: {issues}\n"
        f"Maturity stage: {context.maturity_stage or 'unknown'}\n\n"
        "Generate a personalized business pitch as JSON with these exact keys:\n"
        "- pitch_text: the outreach message\n"
        "- membership_idea: a recurring revenue idea for this business\n"
        "- website_benefits: bullet points on how a new website helps\n"
        "- model_used: set to 'anthropic'\n\n"
        f"Return ONLY valid JSON.{tone_instruction}"
    )


class AnthropicConnector(LLMConnector):
    """Connector for Anthropic API."""

    def __init__(self, model: str | None = None) -> None:
        self.settings = get_settings()
        self.model = model or _DEFAULT_MODEL
        self._client: Any = None

    def _get_client(self) -> Any:
        """Lazy-load the Anthropic client."""
        if self._client is not None:
            return self._client

        try:
            from anthropic import Anthropic
        except ImportError as exc:
            raise ConnectorError("anthropic package is not installed") from exc

        api_key = self.settings.anthropic_api_key
        if not api_key:
            raise ConnectorError("ANTHROPIC_API_KEY is not configured")

        self._client = Anthropic(api_key=api_key)
        return self._client

    def generate_pitch(self, context: BusinessContext, preset: dict | None = None) -> PitchResult:
        """Generate pitch using the Anthropic API."""
        client = self._get_client()
        prompt = _build_prompt(context, preset)
        model = preset.get("model", self.model) if preset else self.model
        system_prompt = preset.get("system_prompt", _DEFAULT_SYSTEM_PROMPT) if preset else _DEFAULT_SYSTEM_PROMPT
        timeout = (
            preset.get("timeout", self.settings.llm_timeout_ms / 1000.0)
            if preset
            else self.settings.llm_timeout_ms / 1000.0
        )

        try:
            response = client.messages.create(
                model=model,
                max_tokens=1024,
                temperature=0.3,
                system=system_prompt,
                messages=[{"role": "user", "content": prompt}],
                timeout=timeout,
            )
        except Exception as exc:
            msg = f"Anthropic API request failed: {exc}"
            if isinstance(exc, _AnthropicTimeoutError):
                msg = "Anthropic API request timed out"
            raise ConnectorError(msg) from exc

        raw = response.content[0].text if response.content else "{}"
        # Anthropic may wrap JSON in markdown blocks
        if "```json" in raw:
            raw = raw.split("```json")[1].split("```")[0].strip()
        elif "```" in raw:
            raw = raw.split("```")[1].split("```")[0].strip()

        try:
            parsed = json.loads(raw)
        except json.JSONDecodeError as exc:
            raise ConnectorError(f"Anthropic response is not valid JSON: {exc}") from exc

        return PitchResult(
            pitch_text=parsed.get("pitch_text", ""),
            membership_idea=parsed.get("membership_idea", ""),
            website_benefits=parsed.get("website_benefits", ""),
            model_used=parsed.get("model_used", "anthropic"),
        )

    def health_check(self) -> bool:
        """Check if the Anthropic API key is configured and has a valid format."""
        key = self.settings.anthropic_api_key
        return bool(key) and key.startswith("sk-ant")

    @classmethod
    def can_use(cls) -> bool:
        """Check if an Anthropic API key is present without instantiating a client."""
        return bool(get_settings().anthropic_api_key)
